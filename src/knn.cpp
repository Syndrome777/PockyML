...
int classification(const vec& data_vec, const int k)
{
	...
	for(int i = 0; i < samp_num; i++){
		dis.push_back(distance(data_vec, raw_data[i]));
	}
	dis_copy = dis;
	...
	int count_num = 0;
	for(it = lab_map.begin(); it != lab_map.end(); it++){
...
vec classification(const mat& data_mat, const int k)
{
	vec resl;
...
inline double distance(const vec& vec1, const vec& vec2)
{
	double var = 0;
